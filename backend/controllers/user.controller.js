...
export const login= async (req, res) => {
    ...
    if(!isPasswordCorrect){
        ...
        if(user.role!==role){
            return res.status(400).json({ message: 'Account doesn\'t exist for this role',
                success: false
             });
        };
        const tokenData={
...
export const logout= async (req, res) => {
    try {
       return res.status(200).cookie("token","",{maxAge:0,httpOnly:true, secure:true, sameSite:'strict'}).json({
